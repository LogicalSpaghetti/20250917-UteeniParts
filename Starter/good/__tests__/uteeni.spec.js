...
describe('E-commerce API (secure behavior expectations)', () => {
    ...
    test('GET /products/search should reject dynamic expressions and only allow safe text', async () => {
        ...
        expect(JSON.parse(res.text).count).toBe(0);

        const resB = await request(app).get('/products/search').query({ q: "name=rust" });
        expect(JSON.parse(resB.text).count).toBe(0);

        const resC = await request(app).get('/products/search').query({ q: "rust" });
        expect(JSON.parse(resC.text).count).toBe(1);
    });
